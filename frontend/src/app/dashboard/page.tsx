          ...
          <div key={s.label} className="rounded-xl bg-[#16162a] border border-white/[0.04] p-4 group hover:border-purple-500/10 transition-colors">
            <div className="flex items-start justify-between">
              ...
              <div className="w-8 h-8 rounded-lg bg-purple-500/[0.06] flex items-center justify-center">
                <s.icon size={15} className="text-purple-400/60" />
              </div>
            </div>
          </div>
        ))}
            ...
            {topPriority.map((r: any, i: number) => (
              <div key={r.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-white/[0.02] transition-colors">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-xs font-medium text-slate-300">{r.asset_name}</p>
                    <p className="text-[10px] text-slate-600">{r.company_name}</p>
                  </div>
                </div>
          ...
          <div className="space-y-1.5">
            {recent.map((r: any) => (
              <div key={r.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-white/[0.02] transition-colors">
                <div>
                  <p className="text-xs font-medium text-slate-300">{r.asset_name}</p>
                  <p className="text-[10px] text-slate-600">{r.issue_type} · {r.location_name}</p>
                </div>
                <div className="text-right">
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium border ${getRiskColor(r.risk_level)}`}>{r.risk_level}</span>
                  <p className="text-[10px] text-slate-600 mt-0.5">{formatDate(r.created_at)}</p>
                </div>
              </div>
            ))}
          </div>
