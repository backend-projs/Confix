...
export const voiceReportRouter = Router();
...
async function callWhisper(audioBuffer: Buffer, lang: string, mimeType: string): Promise<string> {
...
async function callGroq(transcript: string, prompt: string): Promise<any> {
  const key = process.env.GROQ_API_KEY;
  if (!key) throw new Error('GROQ_API_KEY not configured');
...
async function callOpenRouter(transcript: string, prompt: string): Promise<any> {
...
function parseAIResponse(raw: string): { problem_type: string; problem: string; description: string; corrected_transcript: string } {
...
voiceReportRouter.post('/transcribe-and-parse', upload.single('audio'), async (req: Request, res: Response) => {
...
voiceReportRouter.post('/parse', async (req: Request, res: Response) => {
