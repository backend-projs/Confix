...
export function calculateRiskMatrixScore(impact: number, likelihood: number): number {
...
// ── Worker Safety Derivation ──

export interface SafetyFields {
  requiredPPE: string[];
  safetyInstructions: string[];
  workerSafetyLevel: string;
  minimumCrew: number;
  supervisorApprovalRequired: boolean;
  hazardRadiusMeters: number;
}

export function deriveSafetyFields(
  assetType: string,
  issueType: string,
  riskLevel: string
): SafetyFields {
  const ppe: string[] = ['High visibility vest', 'Safety boots'];
  ...
  if (issueType === 'Cable Exposure') {
    if (!ppe.includes('Insulated gloves')) ppe.push('Insulated gloves');
    if (!ppe.includes('Voltage detector')) ppe.push('Voltage detector');
  }
  ...
  // Supervisor approval
  const supervisorApprovalRequired =
    riskLevel === 'High' ||
    riskLevel === 'Critical' ||
    assetType === 'Tunnel' ||
  ...
  if (riskLevel === 'Critical') hazardRadiusMeters = 150;
  else if (riskLevel === 'High') hazardRadiusMeters = 100;
  else if (riskLevel === 'Medium') hazardRadiusMeters = 50;
...
export interface AISuggestion {
  suggestedIssueCategory: string;
  suggestedSummary: string;
  suggestedPPE: string[];
  suggestedSafetyInstructions: string[];
...
export function mockAssistant(
  ...
  const suggestedPPE: string[] = ['High visibility vest', 'Safety boots'];
  const suggestedSafetyInstructions: string[] = [];
...
export function createAuditEvent(action: string, user: string = 'System'): AuditEvent {
  return {
